...
.rm{display:flex;justify-content:space-between;gap:10px;font-family:var(--mono);
font-size:11px;color:var(--dim);padding:3px 0;border-bottom:1px solid var(--line)}
.rm.hd{color:var(--bone);border-bottom-color:var(--line2);margin-top:9px;display:block}
...
@media (max-width:840px){
  ...
  aside{position:absolute;left:0;right:0;bottom:0;width:100%;max-height:74%;
    border-left:0;border-top:1px solid var(--line);background:var(--ink);
    transform:translateY(100%);transition:transform .22s ease;
    padding:14px 16px 28px;z-index:6;
    box-shadow:0 -14px 34px rgba(0,0,0,.5)}
  aside.open{transform:translateY(0)}
  aside::before{content:"";position:absolute;top:6px;left:50%;width:38px;height:4px;
    margin-left:-19px;border-radius:2px;background:var(--line2)}
  #sheetBtn{display:block;position:absolute;right:12px;bottom:12px;z-index:7;
    background:var(--ink2);border:1px solid var(--line);color:var(--bone);
    border-radius:24px;padding:12px 20px;font-size:14px;font-family:var(--sans)}
  .seg button{padding:9px 14px;font-size:13px}
  .row{margin-bottom:12px}
  .row label{width:88px}
  select,input[type=number]{padding:10px 10px;font-size:15px}
  input[type=range]{height:30px}
  input[type=range]::-webkit-slider-thumb{width:22px;height:22px;margin-top:-10px}
  input[type=range]::-moz-range-thumb{width:22px;height:22px}
  .pal button{min-height:46px}
  ...
  #pad.on{display:grid}
  #pad button{background:var(--ink2);border:1px solid var(--line);color:var(--bone);
...
<button id="mR" aria-label="Right">&#9654;</button></div>
</div>
...
<div class="row"><label for="units">Units</label><select id="units">
<option value="m">Metres</option><option value="ft">Feet</option>
<option value="mm">Millimetres</option><option value="in">Inches</option>
</select></div>
<p class="note" id="fsel">Tap any furniture to measure it.</p>
<div class="row"><label for="fedit">Layout</label>
<button class="spec" id="fedit" aria-pressed="false">Move furniture: off</button></div>
<div class="row" id="fdims" hidden><label for="fw">Size</label>
<input type="number" id="fw" step="10" min="100"><span class="stat">&times;</span>
<input type="number" id="fd" step="10" min="100"></div>
<div class="row" id="fbtns" hidden><label>&nbsp;</label>
<button class="spec" id="frot">Rotate 90&deg;</button>
<button class="spec" id="frst">Reset all</button></div>
<div class="row"><label for="glow">Step LEDs</label>
<input type="range" id="glow" min="0" max="250" step="5" value="100">
<span class="stat" id="glowVal">100%</span></div>
<p class="note" id="meta"></p></div>
<div class="grp"><h2>Rooms</h2><div id="rooms"></div></div>
...
const wallMeshes=[];
/* Step-LED materials, with the opacity they were authored at. The slider
...
function applyGlow(){
  glowMats.forEach(e=>{
    e.mat.opacity=Math.min(1,e.base*glowAmt);
    e.mat.visible=glowAmt>0.001;
  });
}
...
function buildStair(g,P,ht){
  ...
  /* LED strip tucked under each nosing. MeshBasicMaterial ignores the scene
  ...
  const glowM=[0.34,0.24,0.15,0.09,0.05].map(o=>{
...
function buildFurniture(g,P){
  ...
  const tag=o=>{if(cur){o.userData.piece=cur; furnMeshes.push(o);
      cur.h=Math.max(cur.h||0,curTop);}   /* tallest point, for the inspector */
    g.add(o); return o;};
  const box=(w,h,d,x,y,z,m)=>{const o=new THREE.Mesh(new THREE.BoxGeometry(w,h,d),m);
...
function build(){
  ...
  /* exposed so a non-clickable piece can be diagnosed from the console */
  window.__model={walls:wallMeshes,furn:furnMeshes,cam:camera,rend:renderer};
  applyFloors();
  applyGlow();
...
let panX=0, panZ=0;    /* orbit target offset, so two fingers can pan */
function frame(){
  if(mode==="orbit"){
    camera.fov=50;
    const ty=((layout==="stack"&&floors==="both")?H.gf*0.6:1.6)*SCALE;
    camera.position.set(panX+radius*Math.sin(phi)*Math.sin(theta), radius*Math.cos(phi)+2,
                        panZ+radius*Math.sin(phi)*Math.cos(theta));
    camera.lookAt(panX,ty,panZ);
  }else{
...
/* Multi-pointer input. One finger orbits or looks; two fingers pinch to zoom
   and drag to pan, which is the only way to zoom on a phone since there is no
   wheel. Pointer events cover mouse, pen and touch with one code path. */
...
const groundAt=e=>{                 /* where a pointer meets the floor plane */
...
renderer.domElement.addEventListener("pointermove",e=>{
  ...
  if(pts.size>=2&&pinch){
    ...
    if(mode==="orbit"){
      ...
      panX-=Math.cos(theta)*mx*k - Math.sin(theta)*my*k;
      panZ+=Math.sin(theta)*mx*k + Math.cos(theta)*my*k;
    }else{ wx-=Math.cos(yaw)*mx*0.012; wz+=Math.sin(yaw)*mx*0.012;
           wx+=Math.sin(yaw)*my*0.012; wz+=Math.cos(yaw)*my*0.012; }
    pinch=s; moved=99; frame(); return;
  }
  if(dragPiece){                    /* slide the meshes, commit on release */
...
addEventListener("keydown",e=>{
  ...
  frame();});
/* ---- furniture inspector ----------------------------------------------- */
const TIER_NAME={sofa:"Sofa",bed:"Bed",tatami:"Tatami platform",table:"Table",
  coffee:"Coffee table",coffeeround:"Coffee table, round",chair:"Chair",
  round:"Side table",bench:"Bench",desk:"Desk",plant:"Planter",rug:"Rug",
  counter:"Worktop run",cupboard:"Full-height cabinet",niche:"Arched niche",
  fridge:"Fridge with cabinet over",applbay:"Appliance cabinet"};
let selPiece=null, selHelper=null, units="m";
function fmtLen(mm){
  if(units==="mm") return Math.round(mm)+" mm";
  if(units==="in") return (mm/25.4).toFixed(1)+" in";
  if(units==="ft"){                       /* feet and inches, as trades quote */
    const t=mm/25.4; let f=Math.floor(t/12), i=Math.round(t-f*12);
    if(i===12){f++;i=0;}
    return f+"' "+i+'"';
  }
...
function fmtArea(m2){
  return units==="ft"||units==="in"
    ? (m2*10.7639).toFixed(0)+" sq ft" : m2.toFixed(1)+" m2";
}
function selectPiece(b){
  selPiece=b;
  ...
  el.innerHTML="<b>"+(TIER_NAME[b[5]]||b[5])+"</b><br>"+
...
function roomList(){
  ...
  ["gf","ff"].forEach(k=>{
    const P=PLAN[k]; if(!P) return;
    h+="<div class=\"rm hd\">"+(k==="gf"?"Ground floor":"First floor")+
       " &mdash; "+fmtLen(P.w)+" &times; "+fmtLen(P.d)+"</div>";
    (P.rooms||[]).slice().sort((a,b)=>b[4]-a[4]).forEach(q=>{
      h+="<div class=\"rm\"><span>"+fmtLen(q[2]-q[0])+" &times; "+fmtLen(q[3]-q[1])+
         "</span><span>"+fmtArea(q[4])+"</span></div>";
    });
  });
  el.innerHTML=h;
}

/* ---- move, rotate and resize furniture ---------------------------------- */
let editMode=false, dragPiece=null, dragFrom=null;
const GPLANE=new THREE.Plane(new THREE.Vector3(0,1,0),0);
const pieceMeshes=b=>furnMeshes.filter(m=>m.userData.piece===b);
const floorOf=b=>((PLAN.gf&&PLAN.gf.furn3d||[]).indexOf(b)>=0)?"gf":"ff";
function editUI(){
  const on=editMode&&!!selPiece;
  document.getElementById("fdims").hidden=!on;
  document.getElementById("fbtns").hidden=!editMode;
  if(on&&selPiece){
    document.getElementById("fw").value=Math.round(selPiece[2]-selPiece[0]);
    document.getElementById("fd").value=Math.round(selPiece[3]-selPiece[1]);
  }
}
/* rewrite a piece's footprint about its own centre, rects included */
function setFootprint(b,w,d){
  const cx=(b[0]+b[2])/2, cz=(b[1]+b[3])/2;
  const sx=w/(b[2]-b[0]), sz=d/(b[3]-b[1]);
  b[4]=b[4].map(r=>[cx+(r[0]-cx)*sx, cz+(r[1]-cz)*sz,
                    cx+(r[2]-cx)*sx, cz+(r[3]-cz)*sz]);
  b[0]=cx-w/2; b[2]=cx+w/2; b[1]=cz-d/2; b[3]=cz+d/2;
}
function nudge(b,dx,dz){
  b[0]+=dx; b[2]+=dx; b[1]+=dz; b[3]+=dz;
  b[4]=b[4].map(r=>[r[0]+dx,r[1]+dz,r[2]+dx,r[3]+dz]);
}
...
["fw","fd"].forEach(id=>document.getElementById(id).onchange=()=>{
  ...
  const w=+document.getElementById("fw").value||selPiece[2]-selPiece[0];
...
document.getElementById("frst").onclick=()=>{
  ...
  build(); selectPiece(null); editUI(); frame();};
...
renderer.domElement.addEventListener("click",e=>{
  ...
  mouse.x=((e.clientX-r.left)/r.width)*2-1; mouse.y=-((e.clientY-r.top)/r.height)*2+1;
  ray.setFromCamera(mouse,camera);
  /* furniture first: if a piece is nearer than any wall, the click is a
     measurement, not a paint stroke */
  const fh=ray.intersectObjects(furnMeshes.filter(m=>m.parent.visible),false)[0];
  const hit=ray.intersectObjects(wallMeshes.filter(m=>m.parent.visible),false)[0];
  if(fh&&(!hit||fh.distance<hit.distance)){
    selectPiece(fh.object.userData.piece); frame(); return;
  }
  if(!hit){ selectPiece(null); frame(); return; }
...
/* ---- mobile: bottom sheet and thumb pad -------------------------------- */
const sheet=document.querySelector("aside"), sheetBtn=document.getElementById("sheetBtn");
sheetBtn.onclick=()=>{
  const open=sheet.classList.toggle("open");
  sheetBtn.setAttribute("aria-expanded",open?"true":"false");
  sheetBtn.textContent=open?"Close":"Controls";
  setTimeout(resize,240);           /* the stage keeps its size, but be safe */
};
/* A phone has no keyboard, so walk mode needs somewhere to press. The pad only
   appears on small screens, and only while walking. */
const pad=document.getElementById("pad");
function padVis(){pad.classList.toggle("on",mode==="walk");}
const step=(fx,fz)=>{const s=.42;
  wx+=Math.sin(yaw)*s*fz+Math.cos(yaw)*s*fx;
  wz+=Math.cos(yaw)*s*fz-Math.sin(yaw)*s*fx; frame();};
[["mF",0,-1],["mB",0,1],["mL",-1,0],["mR",1,0]].forEach(([id,fx,fz])=>{
  const b=document.getElementById(id); let t=null;
  const go=e=>{e.preventDefault(); step(fx,fz); t=setInterval(()=>step(fx,fz),110);};
...
build(); roomList(); sunState(13); resize();
</script>
</body>
</html>'''
